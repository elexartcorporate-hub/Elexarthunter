...
- **Hunter.io Pagination Fix** (Feb 22, 2026) — `hunter_io_search` sebelumnya cap di `limit=25` (1 request), sehingga domain besar (murata.com punya 306+ kontak) hanya kembalikan 25. Sekarang paginated: PAGE_SIZE=100 (max Hunter API), loop `offset=0,100,200,...` sampai habis atau hit safety cap `HUNTER_MAX_RESULTS=500` (env-configurable). Stop jika 429 rate-limit atau page<100 (last page). SMTP probe di-skip untuk email Hunter-sourced (Hunter sudah verifikasi internal; 300+ probes berisiko IP blocklist + 30 menit waktu). Hunter-only emails: `verified` jika `confidence ≥ 80`, else `unverified`. Tested: murata.com → 309 contacts (sebelumnya 4), 187 verified, sample emails include names+titles dari Hunter (Mahesh Subramanian, Tracy Zhu, dll).
