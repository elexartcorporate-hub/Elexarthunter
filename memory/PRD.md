...
- **Source Column di Hasil Search** (Feb 22, 2026) — Tabel "Emails Found" sekarang punya kolom baru **Source** menampilkan asal email: **Playwright** (biru) / **Hunter.io** (biru) / **Playwright + Hunter.io** (hijau, cross-validated, score 100) / **Alias** (abu) / **Playwright (ext)** untuk sibling brand domains. Logic merge confirmed: Playwright crawl run DULU (Step 1-2), Hunter.io setelah itu (Step 3), merge by email key — kalau ada di kedua source, `_sources={"website","hunter"}` → cross-validated → confidence 100 → tampil paling atas (sort desc). Tooltip per badge jelaskan kenapa source itu lebih/kurang trustworthy.
