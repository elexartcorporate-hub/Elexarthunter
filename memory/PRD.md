...
- **WA Health Diagnostic Endpoint + Banner** (Feb 23, 2026) — Bug fix: user di VPS report "Not Found" saat klik Add WhatsApp tanpa info kenapa. Solusi: (1) endpoint baru `GET /api/whatsapp/health` (PUBLIC, no auth) yang ping wa-service via httpx 5s timeout & return `{backend, wa_service_url, wa_service, wa_service_detail}`. (2) Frontend banner `[data-testid='wa-health-banner']` render otomatis kalau wa_service !== "ok" — tampilkan diagnosis + expandable section "📖 Cara fix" dengan shell command lengkap (supervisor config copy, edit secret, install deps, restart, verify). (3) Error handling di handleAdd: 404 → "Backend belum mendukung WhatsApp. Cek WA Health" + 503 → "WA Service tidak jalan. Setup supervisor config" — keduanya trigger loadHealth() ulang. (4) Refresh button juga re-run health. Tested: pytest 26/26 + Playwright 100% (banner OFF saat ok, ON saat unreachable, expand details bekerja, Refresh re-fetch).
