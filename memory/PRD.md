...
- **WhatsApp Multi-Account (Baileys) — Phase 1** (Feb 23, 2026) — Fitur baru menu **WhatsApp** di sidebar. Tiap user bisa add max **3 akun WA** via QR scan. **Owner** bisa melihat (read-only / sales monitoring) semua chat user di tenant — tapi tidak bisa kirim pesan dari WA milik orang lain (403). Persistent across restarts: auth state Baileys disimpan di MongoDB (`wa_auth_creds` + `wa_auth_keys` collections via custom adapter di `wa-service/src/mongoAuthState.js`). Arsitektur: Node.js sidecar service (`/app/wa-service/`, port 3002, Express + Baileys 6.17.16) + FastAPI proxy endpoints + React UI 3-panel (account list + chat list + messages). Auto-reconnect semua sesi saat sidecar start. Frontend: polling /status setiap 2s saat QR, /chats setiap 8s, /messages setiap 5s. Endpoint: `GET/POST /api/whatsapp/accounts`, `GET /api/whatsapp/accounts/{sid}/status`, `DELETE /api/whatsapp/accounts/{sid}`, `GET /api/whatsapp/accounts/{sid}/chats`, `GET/POST /api/whatsapp/accounts/{sid}/chats/{jid}/messages`. Supervisor config: `/etc/supervisor/conf.d/supervisord_wa_service.conf` (pod) + `/app/etc/supervisor.wa-service.conf` (VPS template). `deploy.sh` di-update untuk yarn install + restart wa-service. Test agent: backend 100% (9/9 tests pass), frontend 95% (semua flow verified via Playwright — QR muncul ~1-2s, account card, delete, EmptyState, RBAC enforced).
