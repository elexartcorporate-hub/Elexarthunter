...
- **Daily Target Auto-Sync** (Feb 22, 2026) — Saat user/owner update `daily_target` di Settings (`PATCH /me/target` atau `/team/{uid}/target`), backend sekarang **otomatis update semua task draft/ready** milik user tersebut (`_sync_user_open_task_targets`). Task yang sudah submitted/scheduled/completed TIDAK diubah (history preserved). Plus safety net: `GET /tasks` & `/tasks/{id}` selalu override target field dengan `user.daily_target` current untuk task open (draft/ready). Endpoint `POST /tasks` sekarang menerima target=0 (mode bebas / daily-quest OFF). Frontend Prospects: tab label dan progress card auto-handle target=0 (tampilkan count saja tanpa "/Y", progress bar disembunyikan, email tab terbuka selama ada ≥1 prospect).
